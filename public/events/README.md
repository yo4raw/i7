...
特効ランクごとの UP 率や対象カード数はイベントごとに異なる。具体的な値は各イベント行の `special{N}_*_up` / `special{N}_rID` カラムを参照。
...
| 42-44 | `special{N}_rID` | string | **特効対象のカード ID** のカンマ区切りリスト。親サイトのカード画像 URL および本リポジトリの `card.ID` (`public/assets/cards/{ID}.png`) と同一 |
| 16 | `special{N}_ID` | string | 特効対象の **衣装 ID** のカンマ区切りリスト (ゲーム内の衣装データの識別子。カードの `cardID` などに対応すると思われるが、本リポジトリのカードマスターデータ上の主キーとは異なる) |
...
> [!IMPORTANT]
> カラム名の直感と実データの対応関係に注意。`_ID` の方がメインに見えるが、**実際にカード画像を特定できる ID は `_rID` (= card.ID)** である。親サイト (`event.php?ID=X&view=detail`) の表示を CSV と突き合わせた結果、特効対象カードとして画像付きで表示されるのは `_rID` の値だと確認できている。
>
> - データ上、`_rID` は全 72 イベントで必ず埋まっているが、`_ID` は 26 イベントでのみ埋まっている (46 イベントでは空)
> - 本リポジトリでは `src/lib/data/fetchEventsCsv.ts` が `cardIds` (= `_rID`) と `costumeIds` (= `_ID`) に分けて保持しており、カード画面への紐付けには `cardIds` を使用
...
イベント ID=2 (Re:vale記念日2021、2021-04-15 〜 2021-04-21) の場合:

| 特効 | 効果 | 対象カード ID (`special{N}_rID`) | 対象衣装 ID (`special{N}_ID`) |
|:---:|:---|:---|:---|
| 金特効 | パラメータ +100% / グレードPt +50% / アイテム +30% | `1138,1139,1140,1141` (4 枚) | `1487,1488,1489,1490` |
| 銀特効 | パラメータ +50% / グレードPt +30% / アイテム +20% | `252,253,383,384,...,1086,1087,1088` (17 枚) | `470,471,648,649,...` |
| 銅特効 | パラメータ +10% / グレードPt +10% / アイテム +10% | (なし) | (なし) / `special3_member = "Re:vale"` でグループ指定 |

特効対象カードの画像は `public/assets/cards/{rID}.png` または `public/assets/th_cards/{rID}.png` で参照できる。
