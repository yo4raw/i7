#!/usr/bin/env python3
"""
Complete data import script for all 3 Google Sheets - Denormalized version
"""
import csv
import psycopg2
...
def import_main_cards_denormalized(conn, csv_data):
    """Import main card data in denormalized format"""
    print("\nImporting main card data (denormalized)...")
    cur = conn.cursor()
    ...
    conn.commit()
    print(f"Main cards imported (denormalized): {row_count} rows, {error_count} errors")
    cur.close()

def import_songs_denormalized(conn, csv_data):
    """Import songs data in denormalized format - storing raw CSV rows"""
    print("\nImporting songs data (denormalized)...")
    cur = conn.cursor()
    
    # Create denormalized table for songs that stores raw data
    cur.execute("""
        CREATE TABLE IF NOT EXISTS i7card.songs_denormalized (
            row_id SERIAL PRIMARY KEY,
            raw_data JSONB,
            song_id INTEGER GENERATED ALWAYS AS ((raw_data->>'song_id')::INTEGER) STORED,
            song_name VARCHAR(200) GENERATED ALWAYS AS (raw_data->>'song_name') STORED
        )
    """)
    
    # Create index on song_id for performance
    cur.execute("CREATE INDEX IF NOT EXISTS idx_songs_denorm_song_id ON i7card.songs_denormalized(song_id)")
    
    # Clear existing data
    cur.execute("TRUNCATE TABLE i7card.songs_denormalized")
    ...
    if not headers:
        print("Headers not found in song data")
        return
    ...
    for row in rows[data_start:]:
        try:
            ...
            # Add computed fields
            row_dict['song_id'] = row[0]
            if len(row) > 3:
                row_dict['song_name'] = row[3]
            
            # Store as JSONB
            cur.execute("""
                INSERT INTO i7card.songs_denormalized (raw_data)
                VALUES (%s)
            """, (Json(row_dict),))
    ...
    conn.commit()
    print(f"Songs imported (denormalized): {row_count} rows, {error_count} errors")
    cur.close()

def import_group_cards_denormalized(conn, csv_data):
    """Import group cards data in denormalized format"""
    print("\nImporting group cards data (denormalized)...")
    cur = conn.cursor()
    
    # Create denormalized table for group cards
    cur.execute("""
        CREATE TABLE IF NOT EXISTS i7card.group_cards_denormalized (
            row_id SERIAL PRIMARY KEY,
            raw_data JSONB,
            card_id INTEGER GENERATED ALWAYS AS ((raw_data->>'ID')::INTEGER) STORED,
            cardname VARCHAR(255) GENERATED ALWAYS AS (raw_data->>'cardname') STORED
        )
    """)
    
    # Create index
    cur.execute("CREATE INDEX IF NOT EXISTS idx_group_cards_denorm_card_id ON i7card.group_cards_denormalized(card_id)")
    
    # Clear existing data
    cur.execute("TRUNCATE TABLE i7card.group_cards_denormalized")
    ...
    for row in reader:
        try:
            ...
            # Store entire row as JSONB
            cur.execute("""
                INSERT INTO i7card.group_cards_denormalized (raw_data)
                VALUES (%s)
            """, (Json(dict(row)),))
    ...
    conn.commit()
    print(f"Group cards imported (denormalized): {row_count} rows, {error_count} errors")
    cur.close()
...
def main():
    ...
    try:
        ...
        print("Starting complete denormalized data import...")
        
        # Import main cards
        cards_data = fetch_csv_data(SHEETS["cards"]["gid"])
        import_main_cards_denormalized(conn, cards_data)
        
        # Import songs
        songs_data = fetch_csv_data(SHEETS["songs"]["gid"])
        import_songs_denormalized(conn, songs_data)
        
        # Import group cards
        group_data = fetch_csv_data(SHEETS["group_cards"]["gid"])
        import_group_cards_denormalized(conn, group_data)
        
        print("\n✅ All denormalized data import completed successfully!")
