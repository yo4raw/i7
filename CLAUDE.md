...
Cloudflare Pages (`https://i7.yo4raw.com`) にデプロイ。Git 連携で main push を検知して自動ビルド。

- `wrangler.toml` の `pages_build_output_dir` で Pages 静的配信モードに固定し、Auto Config を回避
- 6 時間ごとの再ビルドは `.github/workflows/cloudflare-rebuild.yml` が Deploy Hook を叩いて実現
