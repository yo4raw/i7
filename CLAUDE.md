...
ユーザー可視テキストの表記は下表に揃える。同じものが 2 通りの名前で呼ばれると利用者が別物と受け取るため、新規 UI・解説文を書くときも必ず左列を使う。

| 使う | 使わない | 備考 |
| ---- | -------- | ---- |
| 固有ブローチ | 固定ブローチ | 衣装に紐づくブローチ（内部識別子は `FixedBroach`） |
| 共通ブローチ | 共有ブローチ | 内部識別子は `sharedBroach` |
| 並び替え | 並べ替え | UI ラベルが「並び替え」 |
| 割り当て / 割り当てる | 割当 | 送り仮名を送る |
| お使いの端末 | 閲覧している端末 | localStorage の保存先の説明 |
| 算術期待値 | 理論期待値 | UI の評価指標ラベルに合わせる。「理論期待値」は定義のない造語 |
