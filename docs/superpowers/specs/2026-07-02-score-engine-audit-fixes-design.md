...
理論最大値はノート毎 floor、期待値は一括 floor のため、per≈100 かつ完全カバーの縮退ケースで期待値が最大値をノーツ数オーダー（数十〜百点、スコア比 0.1% 未満）上回り得る。期待値へ per-note floor を持ち込むと rate 加重と両立せず式が複雑化するため、**許容残差として ADR 0036 に明記**し修正しない。
