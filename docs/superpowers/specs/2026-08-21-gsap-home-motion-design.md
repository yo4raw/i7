...
| 0.12 | 16 色バーの 16 セグメント | `scaleY: 0 → 1`（`transform-origin: bottom`）、1 本あたり 0.4s、stagger 0.025s で左から順に立ち上がる。**`opacity` はフェードさせず 1 のまま**（後述） |
...
**16 色バーだけは `opacity` をフェードさせない。** ヒーローの背景は近黒（`--color-chrome-ink`）で、その上でキャラ色を `opacity 0 → 1` させると、途中のフレームで色が背景に溶けて**暗い矩形**に見える（実装後のスクリーンショットで確認済み）。`opacity` を 1 に固定し `scaleY: 0 → 1` だけを動かすと、出番が来るまでは高さ 0 で見えず、順に**満発色のまま下から立ち上がる**。キャラ色を見せるための演出なので、色が濁る中間状態を作らないことを優先する。
