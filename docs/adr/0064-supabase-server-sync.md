...
既存の `SavedDeck.id` は `Date.now().toString(36)` で UUID ではないため、**サーバ側でも既存 ID を `text` のまま持ち `(user_id, id)` の複合主キーとする**（ID 単独では別ユーザー間で同一ミリ秒の衝突が起きる）。利用者の localStorage 上の ID を書き換えない。`deck_slots` にも `user_id` を持たせ、RLS をサブクエリではなく `auth.uid()` との単純比較で済ませる。
