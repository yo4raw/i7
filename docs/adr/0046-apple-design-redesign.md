...
- 本節は [0054](0054-gsap-home-motion.md) により、**トップページ（`src/pages/index.astro`）に限り GSAP の使用を許可**する形で部分的に上書きされた。それ以外のページでは本節が引き続き有効。無限スクロール行へのエントランス禁止は 0054 でも維持される。
