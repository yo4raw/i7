...
### アプリ側の Web Worker プール数を `hardwareConcurrency - 1` に絞る（見送り）

コアを全部占有すると UI スレッドが飢えるため、実ユーザーにとっても改善になりうる。ただしこれは本番挙動の変更であり、E2E の安定化とは別の意思決定として扱うべき。必要になった時点で別 ADR を起こす。
