...
基本スコア **163,242** に対し、以下のスコアアップ要素が存在する場合は**乗算**で重ねて掛かる:

- SCOREUPアシストが `true` なら ×1.12 を掛ける
  - 適用後スコア: `floor(163,242 × 1.12)` = **182,831**
- SCOREUPバッジ倍率に 15 が入力されていたら ×1.15 を掛ける
  - 適用後スコア: `floor(163,242 × 1.15)` = **187,728**

### 両方同時に適用された場合

アシストとバッジは**加算ではなく乗算**で重ねる:

- 最終スコア: `floor(163,242 × 1.12 × 1.15)` = `floor(210,255.696)` = **210,255**

### 計算例まとめ

| 適用条件 | 倍率 | 最終スコア |
|---------|-----:|----------:|
| なし (基本スコアのみ) | ×1.00 | 163,242 |
| SCOREUPアシストのみ | ×1.12 | 182,831 |
| SCOREUPバッジ (15) のみ | ×1.15 | 187,728 |
| アシスト + バッジ (15) | ×1.12 × ×1.15 = ×1.288 | **210,255** |
