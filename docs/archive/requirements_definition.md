...
> **アーカイブ（2026-08-21）**: 本書はプロジェクト初期の構想を記録したものであり、**現行の仕様ではない**。
> 記載されている画面パス（`/card/[id]`、`/scoreup` 等）やレイアウト方針（「フッターなし」等）は現在の実装と一致しない。
> 現行の構成・仕様は [`CLAUDE.md`](../../CLAUDE.md) と [`docs/adr/`](../adr/README.md) を正とする。
> 本書は当時の意図を追えるようにするためだけに残しており、実装に追随させない。
