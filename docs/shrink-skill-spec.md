...
> 本ドキュメントは判定縮小スキルの **仕様（あるべき挙動）** と、現行実装（`src/lib/score/`）との
> **矛盾点** をまとめたものである。§1–§6 は仕様側を記述し、§7 で実装との差分を列挙する。
> 仕様と実装が食い違う箇所は §7 を一次情報として参照すること。
...
**アシスト適用後の素点** に縮小倍率を乗じる:

```text
noteScoreBase     = floor(attrValue × LIGHT_MULTIPLIER × NOTE_RATE)
noteScoreAssisted = noteScoreBase × (scoreUpAssist ? 1.2 : 1.0)    ← 属性値段階で先に 20% UP
noteScoreShrunk   = floor(noteScoreAssisted × rate)                ← そのあと rate（Lv5 で 1.6）を乗ずる
```

アシスト ON + 縮小中の合計倍率は **素点 × 1.2 × rate**（例: rate=1.6 のとき素点 × 1.92）。
アシスト OFF + 縮小中は **素点 × rate**（例: rate=1.6 のとき素点 × 1.6）。
