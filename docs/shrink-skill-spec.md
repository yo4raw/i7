...
- **効果**: 「すべての縮小スキルが毎回発動した場合」のスコア分布を MC 試行数分得られる。カバー率は単純合計 `Σ floor(eligibleCount / count_i) × value_i` のまま、曲尺 `songDuration` で 100% キャップされる（§3-1 の表示/内部カバー率と同値）。
...
- **7-6 発動位置の線形仮定 vs シャッフル実順序**: `calcShrinkCoverage` は発動時刻を線形計算、`calcMaxScore` / `runOnce` はシャッフル後順序で `counters[c]` を進める。`calcExpectedScore` と MC 平均の微差の原因（7-9 解消後も残る「最初のトリガー成立前のデッドゾーン」と合わせて数% 水準の乖離を生む）。
