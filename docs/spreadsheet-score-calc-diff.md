# スプレッドシート版 (ota-life.com v1.0.6) とのスコア計算ロジック差分

> 比較対象:
>
> - **スプレッドシート**: [アイナナスコア計算 v1.0.6](https://docs.google.com/spreadsheets/d/1PeVXm...09XwI/) (ota-life.com 配布版、コピー ID `1PeVXm...09XwI`)。2026-06-09 採取。
> - **i7 実装**: `src/lib/score/` および `docs/score_calc_spec.md` / `docs/shrink-skill-spec.md`（ブランチ `feat/spreadsheet-oracle` HEAD）
>
> 本差分は `tests/oracle/` の独立オラクル（スプレッドシート v1.0.6 を engine 非依存で忠実移植、ゴールデン#1 で両モード bit-exact 検証済み）と engine 出力を `tests/unit/score/spreadsheetDiff.test.ts` で項目別比較した結果に基づく。
>
> 関連ドキュメント:
>
> - 設計書: `docs/superpowers/specs/2026-06-09-spreadsheet-v1.0.6-oracle-design.md`
> - 調査メモ: `docs/spreadsheet-v1.0.6-investigation.md`
> - ゴールデンフィクスチャ: `tests/fixtures/golden/spreadsheet-v1.0.6.json`
>
> スプレッドシート側の全シート構造・数式リファレンスは `docs/spreadsheet-spec-v1.0.5.md` に分離記録している（v1.0.5 ベース、v1.0.6 での変更は §変更履歴を参照）。本書は **両者の差異と縮小スキル計算の重点比較** に焦点を絞る。
...
v1.0.5 から v1.0.6 への変更は主にイベント楽曲の追加・楽曲データの修正・SCORE UP バッジ初期値の変更が中心であり、スコア計算の数式ロジックの実質的な変更は小さい。

### スコア計算に影響する変更

| 変更箇所 | v1.0.5 | v1.0.6 | 影響 |
| --- | --- | --- | --- |
| `BA11`（Shout 白ノート素点） | `TRUE` リテラル（バグ残置） | `=ROUNDDOWN($AN72*2.5%,0)` 形式に修正 | Shout 白ノート素点が常に 1 になるバグが解消。`sheet5.xml` で確認 |

v1.0.5 の `BA11=TRUE` バグ（Shout 白 per-note score が本来 `floor(Shout × 0.025)` のはずが定数 1 になっていた）は **v1.0.6 で修正済み**。これにより v1.0.6 オラクルの属性値スコアは engine と bit-exact 一致するようになった（v1.0.5 では Shout 白ノート数 × (真の per-note − 1) 分の誤差があった）。

### データ・UI の変更（スコア計算ロジックに非影響）
...
## オラクル検証によるゴールデン#1 実測差分

**ゴールデン#1 条件**:

- デッキ: UR 6 枚（ID 1950 / 1952 / 1493 / 1196 / 1198 / 1362）
- 特効: 銀特効 × 6
- アシスト: ON (`assist=true`)
- バッジ: 16%
- 楽曲: Binary Vampire（461 ノーツ / 92 秒）
- センター: ID 1493 / フレンド: ID 1362

`tests/unit/score/spreadsheetDiff.test.ts` によるオラクル（スプレッドシート v1.0.6 忠実移植）と engine の項目別比較:

| 項目 | モード | オラクル（スプレッドシート） | engine | delta | 分類 |
| --- | --- | ---: | ---: | ---: | --- |
...
センター/フレンド 10% ・特効・per-note 2 段 floor が engine とスプレッドシートで同一の計算経路を持つことを確認。v1.0.5 では `BA11=TRUE` バグにより一致しなかったが、v1.0.6 で修正済み。この項目は回帰ガードの基準点として利用できる。
...
これらの差分はいずれも `tests/oracle/knownDiffs.ts` の許可リストに登録済みであり、テストは意図した差分として通過する。スプレッドシートに合わせるか実機準拠を維持するかは別途サブC フェーズで個別判断する。
...
- `BA11:BF11` = 属性値 × `NOTE_RATE` を `ROUNDDOWN` した「1 ノーツ素点」。v1.0.5 では **`BA11` だけ `TRUE` リテラルが残置されていた**（他の 5 列は `=ROUNDDOWN($AN72*2.5%)` 形式）が、**v1.0.6 で `=ROUNDDOWN($AN72*2.5%,0)` に修正済み**（§変更履歴参照）。
- `BA12:BF17` = 各ステージのライト倍率を乗じて再 floor した per-note 基底値 (`=ROUNDDOWN($BA11 * $AZ12)` 形)

**i7 との per-note 公式**（`NOTE_RATE` / `LIGHT_MULTIPLIER` / 2 段 floor）は一致。v1.0.6 修正後はオラクル検証で属性値スコアが bit-exact 一致することを確認済み。
...
> 下表は v1.0.5 採取数式を元に作成。v1.0.6 での縮小スキル数式の実質変更は無し（§変更履歴参照）。

| # | 項目 | スプレッドシート v1.0.5/v1.0.6 | i7 (`calcExpectedScore` / `shrinkExclusion`) |
| --- | --- | --- | --- |
...
### 5-4. 数値検証: Binary Vampire + 既定デッキ（v1.0.5 採取値）

> v1.0.5 の既定デッキ（百[記念日2024]×2 + 百[7th Anniversary] + 千[記念日2021]×3）での手計算検証。ゴールデン#1 実測値は §「オラクル検証によるゴールデン#1 実測差分」を参照。
...
5. **~~v1.0.5 の `BA11 = TRUE` リテラル残置~~（v1.0.6 修正済み）** → v1.0.5 では Shout 白 notes_20 の per-note score が `1` に固定される不具合があったが、v1.0.6 で `=ROUNDDOWN($AN72*2.5%,0)` に修正された。オラクル検証で属性値スコア bit-exact 一致を確認（§変更履歴参照）。
...
- **`BA11 = TRUE` リテラル問題**: ~~スプレッドシート v1.0.5 は Shout 白 per-note score のセルが `TRUE` になっていた。~~ **v1.0.6 で修正済み**（`=ROUNDDOWN($AN72*2.5%,0)` 形式に）。オラクル検証により属性値スコアが bit-exact 一致することで確認。
