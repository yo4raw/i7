...
describe('MONSTER GENERATiON で 10th 環センター + 記念日2024 環フレンドの2枚構成', () => {
  ...
  describe('スコア計算 (縮小スキルにより分布する)', () => {
    ...
    it('calcExpectedScore.finalScore (算術期待値、仕様 §5-3) は 348,051 と一致', () => {
      // 仕様 §4: 期待縮小時間 = 20×4×0.40 = 32秒
      // effectiveSeconds = 104 × (428−21)/428 ≈ 98.897 (先頭除外 21 ノーツ分を分母から控除)
      // expectedCoverageRate = 32/98.897 ≈ 0.3236
      // shrinkExpected = floor(eligibleBaseScore × (1.6−1.0) × 0.3236)
      const expected = calcExpectedScore(team, notes, notesCount);
      expect(expected.finalScore).toBe(348051);
    });
...
describe('MONSTER GENERATiON で 10th 環センター + 記念日2024 環フレンド + リスポ2 陸 メンバー1 の3枚構成', () => {
  ...
  describe('calcShrinkCoverage (単純合計 + 100% キャップ)', () => {
    ...
    // 実効秒数 = songDuration − 先頭除外秒数 = 104 × (428−21)/428 ≈ 98.897
    const effSeconds = monsterGenerationSong.duration! * (1 - 21 / monsterGenerationSong.notes_count!);
    ...
    it('effectiveSeconds は songDuration − 先頭除外秒数 (offsetSeconds=0)', () => {
      expect(cov!.effectiveSeconds).toBeCloseTo(effSeconds, 5);
    });
  });

  describe('calcShrinkCoverage の実効秒数境界', () => {
    const team = computeTeam(threeCardDeck, tenthTamakiBroachs, monsterGenerationSong);
    const notesCount = monsterGenerationSong.notes_count!;

    it('excludeHeadCount=0 のとき effectiveSeconds は songDuration と一致 (控除なし)', () => {
      const cov = calcShrinkCoverage(team, notesCount, 0, 0);
      expect(cov).not.toBeNull();
      expect(cov!.effectiveSeconds).toBe(monsterGenerationSong.duration);
    });

    it('全ノーツ除外 (excludeHeadCount=notesCount) のときゼロ結果を返す', () => {
      const cov = calcShrinkCoverage(team, notesCount, 0, notesCount);
      expect(cov).not.toBeNull();
      expect(cov!.coverageRate).toBe(0);
      expect(cov!.effectiveSeconds).toBe(0);
    });

    it('notesCount=0 のときゼロ結果 (effectiveSeconds は songDuration のまま)', () => {
      const cov = calcShrinkCoverage(team, 0, 0, 0);
      expect(cov).not.toBeNull();
      expect(cov!.coverageRate).toBe(0);
      expect(cov!.effectiveSeconds).toBe(monsterGenerationSong.duration);
    });
...
/** 平凡な日常3 和泉一織 (SR / スコアアップコンボ / Lv5 は未整備で 0) */
const fallbackSkillLevelCard = findCardById(78);
const fallbackSkillLevelDeck: (Card | null)[] = [fallbackSkillLevelCard, null, null, null, null, null];

describe('Lv5 が 0 のスキルデータは最大有効Lvへフォールバックする', () => {
  const team = computeTeam(fallbackSkillLevelDeck, [], monsterGenerationSong, undefined, undefined, undefined, undefined, [5, 5, 5, 5, 5, 5]);
  const notes = flattenNotes(monsterGenerationSong, FLATTEN_SEED);

  it('Lv5 指定でも Lv4 の有効なスキル値を採用する', () => {
    const skill = team.cards[0].skill;
    expect(skill).not.toBeNull();
    expect(skill!.isShrink).toBe(false);
    expect(skill!.isTimer).toBe(false);
    expect(skill!.count).toBe(17);
    expect(skill!.per).toBe(39);
...
/** 音に想いをのせて 七瀬陸 (UR / 判定縮小タイマー / Lv5 count=23秒) */
const shrinkTimerCard = findCardById(976);
const shrinkTimerDeck: (Card | null)[] = [shrinkTimerCard, null, null, null, null, null];

describe('判定縮小（タイマー）は秒数ベースの縮小スキルとして扱う', () => {
  const team = computeTeam(shrinkTimerDeck, [], monsterGenerationSong);
  const exclusion = computeShrinkExclusion(team, computeGroupSizes(monsterGenerationSong));
  const notes = flattenNotes(monsterGenerationSong, FLATTEN_SEED, exclusion);
  const notesCount = monsterGenerationSong.notes_count!;

  it('スコアアップタイマーではなく縮小タイマーとして解析される', () => {
    const skill = team.cards[0].skill;
    expect(skill).not.toBeNull();
    expect(skill!.isShrink).toBe(true);
    expect(skill!.isTimer).toBe(true);
    expect(skill!.count).toBe(23);
    expect(skill!.per).toBe(38);
    expect(skill!.value).toBe(4);
    expect(skill!.rate).toBe(1.6);
  });

  it('最大発動回数は notes/count ではなく songDuration/count で計算する', () => {
    expect(calcCardSkillMaxActivations(team, notesCount, 0)).toBe(Math.floor(monsterGenerationSong.duration! / 23));
  ...
  it('縮小全発動モードの MC 発動回数も秒数ベースになる', async () => {
    ...
    const stat = result.cardStats.find((s) => s.cardIndex === 0);
    expect(stat).toBeDefined();
    expect(stat!.avgActivations).toBe(4);
  });
});
