...
describe('fetchSheetRaw (fetch モック)', () => {
  it('正常レスポンスでテーブルを返す', async () => {
    vi.stubGlobal('fetch', vi.fn(() => ({
      ...
      text: () => Promise.resolve(gvizText([{ c: [{ v: 1 }] }])),
    })));
  ...
  it('response.ok=false ならリトライ後に throw', async () => {
    vi.stubGlobal('fetch', vi.fn(() => ({
      ok: false, status: 500, statusText: 'Server Error', text: () => Promise.resolve(''),
    })));
  ...
  it('空データ（rows=0）なら throw', async () => {
    vi.stubGlobal('fetch', vi.fn(() => ({
      ok: true, status: 200, statusText: 'OK', text: () => Promise.resolve(gvizText([])),
    })));
    const p = fetchSheetRaw('sid', 1, 1);
  ...
  it('1 回失敗→2 回目成功でリトライが効く', async () => {
    const fetchMock = vi.fn()
      .mockResolvedValueOnce({ ok: false, status: 503, statusText: 'busy', text: () => Promise.resolve('') })
      .mockResolvedValueOnce({ ok: true, status: 200, statusText: 'OK', text: () => Promise.resolve(gvizText([{ c: [{ v: 1 }] }])) });
...
describe('fetchSheetAsJson (fetch モック)', () => {
  it('cols.label をヘッダにし column_ 列はスキップする', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn(() => ({
      ok: true, status: 200, statusText: 'OK',
      text: () => Promise.resolve(gvizText([{ c: [{ v: '陸' }, { v: 'skip' }] }], cols)),
    })));
