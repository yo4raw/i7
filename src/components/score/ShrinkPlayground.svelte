...
<div class="border border-gray-200 dark:border-slate-700 rounded-lg p-4 bg-gray-50 dark:bg-slate-900 space-y-4">
  <div class="flex items-center justify-between flex-wrap gap-2">
    <h4 class="text-sm font-bold text-gray-800 dark:text-slate-100">縮小スキル {cards.length} 枚構成</h4>
  ...
  {#each cards as card, i (i)}
    <div class="bg-white dark:bg-slate-800 rounded border border-gray-200 dark:border-slate-700 p-3">
      <div class="flex items-center justify-between mb-2">
        ...
        {#if cards.length > 1}
          <button
            type="button"
            class="text-xs text-gray-500 dark:text-slate-400 hover:text-red-600"
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
        <label class="flex flex-col gap-1">
          <span class="font-semibold text-gray-700 dark:text-slate-200">
            ノーツ数 <span class="text-gray-500 dark:text-slate-400">(count)</span>:
            <span class="text-indigo-600">{card.count}</span>
          </span>
          <input type="range" min="10" max="30" step="1" bind:value={card.count} class="w-full" />
          <span class="text-gray-500 dark:text-slate-400">何ノーツ毎に発動判定するか</span>
        </label>
        <label class="flex flex-col gap-1">
          <span class="font-semibold text-gray-700 dark:text-slate-200">
            確率 <span class="text-gray-500 dark:text-slate-400">(per)</span>:
            <span class="text-indigo-600">{card.per}%</span>
          </span>
          <input type="range" min="5" max="80" step="1" bind:value={card.per} class="w-full" />
          <span class="text-gray-500 dark:text-slate-400">判定ごとに発動する確率</span>
        </label>
        <label class="flex flex-col gap-1">
          <span class="font-semibold text-gray-700 dark:text-slate-200">
            秒数 <span class="text-gray-500 dark:text-slate-400">(value)</span>:
            <span class="text-indigo-600">{card.value}秒</span>
          </span>
          <input type="range" min="1" max="8" step="1" bind:value={card.value} class="w-full" />
          <span class="text-gray-500 dark:text-slate-400">発動時の持続秒数</span>
        </label>
      </div>
    </div>
  {/each}

  <div class="flex items-center gap-3 text-xs text-gray-600 dark:text-slate-300 flex-wrap">
    <label class="flex items-center gap-1">
      <span class="font-semibold text-gray-700 dark:text-slate-200">倍率 <span class="text-gray-500 dark:text-slate-400">(rate)</span>:</span>
    ...
    <span>先頭除外: <strong class="text-gray-800 dark:text-slate-100">{excludeHead}</strong></span>
    <span>
      発動合計: <strong class="text-gray-800 dark:text-slate-100">{firedCount}/{totalTriggers}</strong>
    </span>
    <span>
      カバー率: <strong class="text-gray-800 dark:text-slate-100">{coveragePct}%</strong>
