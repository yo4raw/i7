<script lang="ts">
  ...
  // 初期値: 典型的な縮小衣装 (count=20 / per=40% / value=4秒) × 1 枚
  ...
  let coveredSec = $derived(
    activations.filter((a) => a.fired).reduce((sum, a) => {
...
<div class="border border-orange-200 rounded-lg p-4 bg-orange-50/40 space-y-4">
  <div class="flex items-center justify-between flex-wrap gap-2">
    <h4 class="text-sm font-bold text-gray-800">縮小スキル {cards.length} 枚構成</h4>
    <div class="flex items-center gap-2">
      <button
        type="button"
        class="px-2 py-1 text-xs rounded border border-orange-500 text-orange-700 hover:bg-orange-100 disabled:opacity-40 disabled:cursor-not-allowed"
        onclick={addCard}
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
        <label class="flex flex-col gap-1">
          <span class="font-semibold text-gray-700">
            ...
            <span class="text-orange-700">{card.count}</span>
          </span>
          <input type="range" min="10" max="30" step="1" bind:value={card.count} class="w-full accent-orange-600" />
          <span class="text-gray-500">何ノーツ毎に発動判定するか</span>
        </label>
        <label class="flex flex-col gap-1">
          <span class="font-semibold text-gray-700">
            確率 <span class="text-gray-500">(per)</span>:
            <span class="text-orange-700">{card.per}%</span>
          </span>
          <input type="range" min="5" max="80" step="1" bind:value={card.per} class="w-full accent-orange-600" />
          <span class="text-gray-500">判定ごとに発動する確率</span>
        </label>
        <label class="flex flex-col gap-1">
          <span class="font-semibold text-gray-700">
            秒数 <span class="text-gray-500">(value)</span>:
            <span class="text-orange-700">{card.value}秒</span>
          </span>
          <input type="range" min="1" max="8" step="1" bind:value={card.value} class="w-full accent-orange-600" />
  ...
  <p class="text-xs text-gray-500">
    ※ 複数枚構成時は <strong>キューイング仕様</strong>（§4 冒頭参照）に従い、先行スキルが終了してから後続スキルが連続発動します（時間軸上で重なりません）。
  </p>
</div>
