    ...
    <div class="relative max-w-2xl mx-auto mt-8 mb-8 bg-white rounded-lg shadow-xl max-h-[85vh] flex flex-col mx-4 sm:mx-auto">
      <div class="p-4 border-b flex-shrink-0">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
          ...
          <select id="modal-attribute" class="border border-gray-300 rounded px-2 py-1.5 text-sm" bind:value={attribute}>
        ...
        <div class="mt-2 flex items-center gap-3">
          ...
          <span id="modal-result-count" class="text-xs text-gray-500">{filtered.length}件</span>
        </div>
      </div>
      <div id="modal-card-list" class="overflow-y-auto flex-1 p-2">
        {#each filtered.slice(0, 200) as card (card.ID)}
          <!-- svelte-ignore a11y_click_events_have_key_events a11y_no_static_element_interactions -->
          <div class="flex items-center gap-2 p-2 rounded hover:bg-gray-50 cursor-pointer border-b border-gray-100" data-pick-card={card.ID} onclick={() => pick(card)}>
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="text-[10px] text-gray-500">{card.name || ''} | 合計: {((card.shout_max || 0) + (card.beat_max || 0) + (card.melody_max || 0)).toLocaleString()} | {card.ap_skill_type || '-'}</div>
            </div>
          </div>
        {:else}
          <p class="text-sm text-gray-400 text-center py-8">該当する衣装がありません</p>
        {/each}
      </div>
