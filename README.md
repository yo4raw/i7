...
- コミットせず取得だけしたいときは `node scripts/fetch-song-images.mjs` を直接実行します
- 新規画像が無ければ「新しい楽曲画像はありません」と表示して終了します
- push・PR・リリースは通常の手順（`develop` へ PR → `main` へ fast-forward）で行います
- 曲名が自動で Wiki ページに一致しない場合は `scripts/fetch-song-images.mjs` 冒頭の手動オーバーライド表に対応を追加します
